...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if self.use_batchnorm:
            self.params["gamma1"] = 1 + np.zeros(hidden_dims[0])
        ...
        for i in range(2, self.num_layers):
            ...
            if self.use_batchnorm:
                self.params["gamma" + str(i)] = 1 + np.zeros(hidden_dims[i-1])
        ...
        # When using dropout we need to pass a dropout_param dictionary to each hidden_dims[self.num_layers]
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_batchnorm:
                ...
                a, cache = batchnorm_forward(a, gamma, beta, self.bn_params[i-1])
                caches.append(cache)

            a, cache = relu_forward(a)
            caches.append(cache)
        ...
        da, dW, db = affine_backward(dscores, caches.pop())
        grads['W' + str(self.num_layers)] = dW + self.reg * self.params['W' + str(self.num_layers)]
        grads['b'+ str(self.num_layers)] = db
        loss += 0.5 * self.reg * np.sum(self.params['W' + str(self.num_layers)]**2)
        for i in range(self.num_layers - 1, 0,-1):
            if self.use_dropout:
                da = dropout_backward(da, caches.pop())

            da = relu_backward(da, caches.pop())
            ...
            da, dW, db = affine_backward(da, caches.pop())
